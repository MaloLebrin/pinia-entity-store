...
import type { FilterFn, Id, State, WithId } from '../types'

export function createGetters<T extends WithId>(currentState: State<T & { $isDirty: boolean }>) {
  /**
  ...
  /**
   * Get all the items from the state as a dictionary of values.
   */
  ...
  /**
   * Returns a boolean indicating whether or not the state is empty (contains no items).
   */
  ...
  /**
   * Returns a boolean indicating whether or not the state is not empty (contains items).
   */
  ...
  /**
   * Get an array of items from the state by their ids.
